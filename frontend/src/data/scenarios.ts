import type { Scenario, CandidateVessel } from '../types/dashboard';

export const SCENARIOS: Record<string, Scenario> = {
  'INC-001': {
    id: 'INC-2026-001',
    title: 'Mumbai High Offshore Basin',
    sub: 'Mumbai High Offshore Basin · Arabian Sea EEZ · 18.743°N, 71.218°E',
    lat: 18.743,
    lng: 71.218,
    oilType: 'Crude Oil',
    oilColor: '#B45309',
    oilFill: 'rgba(180, 83, 9, 0.45)',
    sev: 'CRITICAL SEVERITY',
    sevClass: 'chip-c',
    area: '4.82 km²',

    topVessel: 'CRUDE ATLAS',
    diagVessel: 'CRUDE ATLAS · MMSI 419001234',
    diagDetails: 'Transponder Silent: 04:17–08:52 UTC (4h 35m) · Speed drop: 13.2kn → 4.1kn in origin envelope',
    scores: [0.82, 0.61, 0.34]
  },
  'INC-002': {
    id: 'INC-2026-002',
    title: 'Chennai–Ennore Coastal Corridor',
    sub: 'Chennai–Ennore Anchorage Fairway · Bay of Bengal EEZ · 13.250°N, 80.460°E',
    lat: 13.250,
    lng: 80.460,
    oilType: 'Heavy Bunker Fuel',
    oilColor: '#0D0D11',
    oilFill: 'rgba(13, 13, 17, 0.70)',
    sev: 'HIGH SEVERITY',
    sevClass: 'chip-h',
    area: '2.40 km²',
    topVessel: 'PACIFIC GLORY',
    diagVessel: 'PACIFIC GLORY · MMSI 419009988',
    diagDetails: 'Bunker fuel collision trail · AIS intermittent 02:10–05:40 UTC',
    scores: [0.68, 0.54, 0.28]
  },
  'INC-003': {
    id: 'INC-2026-003',
    title: 'Andaman Sea Shipping Lane 7',
    sub: 'Andaman Sea SL-7 · Malacca Route Approach · 10.456°N, 93.123°E',
    lat: 10.456,
    lng: 93.123,
    oilType: 'Oil Bilge Water',
    oilColor: '#38BDF8',
    oilFill: 'rgba(56, 189, 248, 0.45)',
    sev: 'MEDIUM SEVERITY',
    sevClass: 'chip-m',
    area: '0.95 km²',
    topVessel: 'UNKNOWN (DARK VESSEL)',
    diagVessel: 'TRANSPONDER BLACKOUT · SAR IDENTIFIED',
    diagDetails: 'Metallic SAR return detected in slick centroid with NO matching AIS transmission',
    scores: [0.74, 0.42, 0.19]
  },
  'INC-004': {
    id: 'INC-2026-004',
    title: 'Goa Coastal Waters (Bunkering Leak)',
    sub: 'Goa Coastal Waters (Offshore Anchorage) · 15.420°N, 73.650°E · Arabian Sea',
    lat: 15.420,
    lng: 73.650,
    oilType: 'Diesel / Marine Gas Oil',
    oilColor: '#EAB308',
    oilFill: 'rgba(234, 179, 8, 0.45)',
    sev: 'LOW SEVERITY',
    sevClass: 'chip-l',
    area: '1.75 km²',
    topVessel: 'SEA PEARL',
    diagVessel: 'SEA PEARL · MMSI 419003322',
    diagDetails: 'Marine Gas Oil sheen from bunkering hose failure · Continuous AIS broadcast',
    scores: [0.55, 0.38, 0.21]
  },
  'INC-005': {
    id: 'INC-2026-005',
    title: 'Gulf of Kutch / Vadinar Deepwater Terminal',
    sub: 'Vadinar Single Point Mooring (SPM) Fairway · Gulf of Kutch · 22.600°N, 69.500°E',
    lat: 22.600,
    lng: 69.500,
    oilType: 'Crude Oil',
    oilColor: '#B45309',
    oilFill: 'rgba(180, 83, 9, 0.45)',
    sev: 'HIGH SEVERITY',
    sevClass: 'chip-h',
    area: '3.85 km²',
    topVessel: 'AL KHALEEJ STAR',
    diagVessel: 'AL KHALEEJ STAR · MMSI 419008811',
    diagDetails: 'Offshore SPM hose flange leakage during crude offloading · AIS active',
    scores: [0.78, 0.52, 0.31]
  },
  'INC-006': {
    id: 'INC-2026-006',
    title: 'Cochin Port SPM Anchorage',
    sub: 'Cochin Port SPM Anchorage · Arabian Sea Deepwater Fairway · 9.960°N, 76.080°E',
    lat: 9.960,
    lng: 76.080,
    oilType: 'Heavy Bunker Fuel',
    oilColor: '#0D0D11',
    oilFill: 'rgba(13, 13, 17, 0.70)',
    sev: 'MEDIUM SEVERITY',
    sevClass: 'chip-m',
    area: '2.10 km²',
    topVessel: 'OCEAN VOYAGER',
    diagVessel: 'OCEAN VOYAGER · MMSI 419004455',
    diagDetails: 'Bunker fuel overflow during vessel bunkering transfer · Speed 1.2kn',
    scores: [0.63, 0.45, 0.22]
  },
  'INC-007': {
    id: 'INC-2026-007',
    title: 'Paradip Port Offshore Basin',
    sub: 'Paradip Port Offshore Anchorage · Bay of Bengal EEZ · 20.250°N, 86.720°E',
    lat: 20.250,
    lng: 86.720,
    oilType: 'Crude Oil',
    oilColor: '#B45309',
    oilFill: 'rgba(180, 83, 9, 0.45)',
    sev: 'HIGH SEVERITY',
    sevClass: 'chip-h',
    area: '3.45 km²',
    topVessel: 'EASTERN GLORY',
    diagVessel: 'EASTERN GLORY · MMSI 419006677',
    diagDetails: 'Deepwater single-buoy mooring pipeline pressure drop anomaly',
    scores: [0.72, 0.58, 0.29]
  },
  'INC-008': {
    id: 'INC-2026-008',
    title: 'Lakshadweep Sea 9-Degree Channel',
    sub: '9-Degree Channel · International Tanker Route · 8.500°N, 73.000°E',
    lat: 8.500,
    lng: 73.000,
    oilType: 'Oil Bilge Water',
    oilColor: '#38BDF8',
    oilFill: 'rgba(56, 189, 248, 0.45)',
    sev: 'LOW SEVERITY',
    sevClass: 'chip-l',
    area: '1.30 km²',
    topVessel: 'UNKNOWN (TRANSIT)',
    diagVessel: 'INTERMITTENT TRANSPONDER · WESTBOUND VLCC',
    diagDetails: 'Intermittent bilge discharge along East-West international fairway',
    scores: [0.59, 0.35, 0.18]
  }
};

export const INITIAL_CANDIDATE_VESSELS: CandidateVessel[] = [
  {
    name: 'CRUDE ATLAS',
    mmsi: '419001234',
    flag: 'India',
    type: 'Crude Oil Tanker',
    score: 0.82,
    distScore: 0.92,
    timeScore: 0.88,
    gapScore: 0.75,
    typeScore: 0.90,
    aisStatus: 'CPA: 1.2 nm from envelope centroid · Speed: 4.1 kn · AIS Gap: 4h 35m (suspicious)',
    risk: 'CRITICAL'
  },
  {
    name: 'MARITIME KOHISTAN',
    mmsi: '419005678',
    flag: 'India',
    type: 'Product Tanker',
    score: 0.61,
    distScore: 0.65,
    timeScore: 0.70,
    gapScore: 0.50,
    typeScore: 0.80,
    aisStatus: 'CPA: 3.8 nm from centroid · Speed: 12.4 kn · AIS Gap: 3h 33m',
    risk: 'HIGH'
  },
  {
    name: 'GULF NAVIGATOR',
    mmsi: '419007890',
    flag: 'Panama',
    type: 'Chemical Tanker',
    score: 0.34,
    distScore: 0.30,
    timeScore: 0.40,
    gapScore: 0.25,
    typeScore: 0.70,
    aisStatus: 'CPA: 7.1 nm from centroid · Speed: 13.7 kn · AIS Gap: 1h 22m (normal shadow)',
    risk: 'LOW'
  }
];
